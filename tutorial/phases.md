...
инструкции обьединяются в блоки, находятся переходы между блоками.
...
```LLVM
; испльзуется регистров целочисленных ; с плавающей запятой
start:
%n = ? ; 1 ; 0
%a = ? ; 1 ; 1
%d = ? ; 1 ; 2
...
уменьшить использование медленной памяти для хранения значений, не помещяющихся
в регистры.
На этом этапе можно провести много типов оптимизации, например, современные
процессоры имеют несколько арифметико-логических устройтв, поэтому они могут
...
вычислений.
Мы ограничемся преобразованием программы из SSA формы в машинный инструкции,
...
будут хранится инструкции и не оформили код в виде исполняесого модуля,
который может запустить операционная система.
Эти шаги делаются на последнем этапе, которые часто отделяют от коспиляции
и назывют сборкой.
Компиляция, осбенно с оптимизацией, является весьма ресурсоемкой операцией,
поэтому обычно ее выполняют не для всех программы целиком, а для отдельных
ее частей: модулей или классов.
Сборка же является относительно простой операцией (если не выполняется
межмодульная оптимизация), поэтому ее можно эффективно провести сразу над
всеми модулями.
Результат компиляции в этом случае назывют объектным файлом,
а результат сборки исполняемым модулем. 
Отделение этапа сборки от компиляции также позволяет распространять 
программные библиотеки не распростряняя исходный код.
Для этого создается библиотечный файл (dll, so и др.),
содержащий обьектный код и названия фунций.
При сборке кода ссылки в пользовательском коде на имена функций заменяются 
...
Разобрав последовательность действий, выполняемых компиляторо, мы пришли
к следующим фазам компиляции:

* Синтаксический анализ и генерация AST.
* Проверка корректости программы, генерация вспомогательных структур.
...
один кодогенератор.
Сборщик может собирать обьектный код, сгенерированный компиляторами
разных языков.
...
* Что такое обьектный код? Зачем нужна сборка?
