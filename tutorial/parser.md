...
последовательно по одному нетерминальному символу на правые части правил,
причем символы заменяются независимо друг от друга, независимо от контекста.
Слово из контекстно-свободного языка можно разбить на подстроки,
...
Мы пронумеровали правила, чтобы помечать узлы дерева разбора,
...
а терминал `+` в дереве не отражается, так как он однозначно определен правилом `2`.
...
Многие генераторы синтаксических анализаторов могут автоматически генерировать
вспомогательные нетерминалы, программисту же остается только задать
...
поэтому из AST эти узлы удаляют.

Добавим теперь в наши арифметические выражения скобки.
Скобки всегда встречаются парой, поэтому и добавляться они должны парой
в одном правиле.
...
Один из самых известных генераторов парсеров
